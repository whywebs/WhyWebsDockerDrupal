...
RUN set -ex \
	...
	&& echo "# Added for xdebug" >> /etc/php.ini \
	&& echo "extension=xdebug.so" >> /etc/php.ini \
	&& echo "xdebug.remote_enable=1" >> /etc/php.ini \
	&& echo "xdebug.remote_handler=dbgp xdebug.remote_mode=req" >> /etc/php.ini \
	&& echo "xdebug.remote_host=127.0.0.1 xdebug.remote_port=9000" >> /etc/php.ini \
